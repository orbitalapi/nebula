...
| `--network`      | `NEBULA_NETWORK`     | `nebula_network` | Identifies the docker network to attach started containers to. Only used in `network` mode (see below); in `host` mode an isolated network is always created |
